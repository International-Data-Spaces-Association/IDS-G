...
|**`iss`**                  | yes    | 1    | According to RFC 7519 Sec. 4.1.1, the issuer is the component which created and signed the JWT. In the context of the IDS, this must be a valid connector. The value of `iss`  must be the combined entry of the [Subject Key Identifier (SKI)](#subject-key-identifier-ski) and [Authority Key Identifier (AKI)](#authority-key-identifier-aki) of the Connectors X509 certificate as presented in Sec. 4.2.1.  |
